...
class OrderBook {
   public:
    // Matches against the opposite side, then rests any remainder (limit orders only)
    // Trades are appended to the "trades" vector
    [[nodiscard]] ErrorCode submit(const Order& order, std::vector<Trade>& trades);
    [[nodiscard]] ErrorCode cancel(OrderId id);
...
template <typename ContraSideLevels>
void OrderBook::match(Order& order, ContraSideLevels& contra_side_levels,
